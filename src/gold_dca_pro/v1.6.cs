...
// Version: 1.6 (cTrader)   Date: 2026-04-23
// Platform: cTrader / cAlgo (C#)
// ============================================================================
// CHANGELOG:
// v1.6-cT (2026-04-23) — Added WIN STREAK AMPLIFIER system
//                         Tracks consecutive winning baskets
//                         2 wins → 1.5x lot size, +1 max position, wider TP buffer
//                         3 wins → 2.0x lot size, +2 max positions, even wider TP
//                         Any stop-out resets win counter
//                         Dashboard shows win streak status
//                         WARNING: Pro-cyclical — amplifies wins AND losses
//
...
namespace cAlgo.Robots
{
    ...
    public class GoldDCAPro : Robot
    {
        ...
        [Parameter("Use Win Streak Amplifier", DefaultValue = true, Group = "Win Streak")]
        public bool UseWinStreak { get; set; }

        [Parameter("2-Win: Lot Multiplier", DefaultValue = 1.5, Group = "Win Streak")]
        public double Win2LotMult { get; set; }

        [Parameter("2-Win: Extra Max Positions", DefaultValue = 1, Group = "Win Streak")]
        public int Win2ExtraPos { get; set; }

        [Parameter("2-Win: Extra TP Buffer (pips)", DefaultValue = 30, Group = "Win Streak")]
        public int Win2ExtraTP { get; set; }

        [Parameter("3-Win: Lot Multiplier", DefaultValue = 2.0, Group = "Win Streak")]
        public double Win3LotMult { get; set; }

        [Parameter("3-Win: Extra Max Positions", DefaultValue = 2, Group = "Win Streak")]
        public int Win3ExtraPos { get; set; }

        [Parameter("3-Win: Extra TP Buffer (pips)", DefaultValue = 60, Group = "Win Streak")]
        public int Win3ExtraTP { get; set; }
        ...
        private double _balanceBeforeLastClose = 0;
        ...
        protected override void OnStart()
        {
            ...
            Print("[GoldDCA Pro v1.6-cT] Started on {0} | Balance: £{1:F2} | Tier: {2} ({3})",
        ...
        protected override void OnTick()
        {
            ...
            // Get win streak bonuses
            var winBonus = GetCurrentWinBonus();
            int effectiveMaxPositions = tier.MaxPositions + winBonus.ExtraPositions;
            int effectiveMinProfit = MinProfitPips + winBonus.ExtraTPPips;
            ...
            if (b >= 2)
            {
                double weighted = (buyPriceMax * buyVolMax + buyPriceMin * buyVolMin) /
                                  (buyVolMax + buyVolMin);
                averageBuyPrice = weighted + effectiveMinProfit * Symbol.PipSize;
            }

            // Next lot — tier-driven, with win streak multiplier
            double baseStartLot = tier.StartLots * winBonus.LotMult;
            double nextLots = (buyVolMin == 0) ? baseStartLot : VolumeToLots(buyVolMin) * 2;
            ...
            // Detect basket completion — check transition from positions open to closed
            if (_wasInSessionLastCheck && b == 0)
            {
                // Compare current balance to what we captured while basket was open
                // If current > pre-close → win; if current < pre-close → loss
                double delta = Account.Balance - _balanceBeforeLastClose;
                OnBasketClosed(delta);
            }

            // Continuously update pre-close snapshot while basket is open
            if (b > 0)
                _balanceBeforeLastClose = Account.Balance;
            ...
            // --- OPEN NEW BUY LEVELS ---
            if (inSession && !_scenarioClosed && !inCooldownPause && b < effectiveMaxPositions)
            {
                ...
                if (b > 0 && b < effectiveMaxPositions)
                {
        ...
        private void OnBasketClosed(double balanceDelta)
        {
            // Positive delta = win, negative or zero = loss
            bool isWin = balanceDelta > 0;

            if (isWin)
            {
                _consecutiveWins++;
                Print("[GoldDCA Pro] ✅ Winning basket (+£{0:F2}) | Win streak: {1}",
                      balanceDelta, _consecutiveWins);

                // Reset cooldown state on any win
                if (UseCooldown && (_recentStopOuts.Count > 0 || _skipNextSession || _cooldownUntil > Server.Time))
                {
                    Print("[GoldDCA Pro] ✅ Win — cooldown counters reset");
            ...
            {
                // Loss resets win streak (stop-out already handled in RegisterStopOut)
                if (_consecutiveWins > 0)
                {
                    Print("[GoldDCA Pro] Win streak broken (was {0})", _consecutiveWins);
                }
                _consecutiveWins = 0;
            }
        }

        // --- WIN STREAK SETTINGS ACCESS ---

        private class WinBonus
        {
            public double LotMult = 1.0;
            public int ExtraPositions = 0;
            public int ExtraTPPips = 0;
        }

        private WinBonus GetCurrentWinBonus()
        {
            var bonus = new WinBonus();
            if (!UseWinStreak) return bonus;

            if (_consecutiveWins >= 3)
            {
                bonus.LotMult = Win3LotMult;
                bonus.ExtraPositions = Win3ExtraPos;
                bonus.ExtraTPPips = Win3ExtraTP;
            }
            else if (_consecutiveWins >= 2)
            {
                bonus.LotMult = Win2LotMult;
                bonus.ExtraPositions = Win2ExtraPos;
                bonus.ExtraTPPips = Win2ExtraTP;
            }
            return bonus;
        }
        ...
        private void UpdateDashboard(TierSettings tier)
        {
            ...
            {
                if (_consecutiveWins >= 3)
                    winStatus = string.Format("🔥 3+ wins (x{0} lots, +{1} pos)", winBonus.LotMult, winBonus.ExtraPositions);
                else if (_consecutiveWins >= 2)
                    winStatus = string.Format("📈 2 wins (x{0} lots, +{1} pos)", winBonus.LotMult, winBonus.ExtraPositions);
                else if (_consecutiveWins == 1)
                    winStatus = "1 win";
                else
                    winStatus = "NORMAL";
            }

            string dash = "════ GoldDCA Pro v1.6-cT ════\n";
            ...
            dash += "\n";
            dash += string.Format("  Start Lot: {0}", tier.StartLots);
            if (winBonus.LotMult > 1.0)
                dash += string.Format(" → {0:F3} (x{1})", tier.StartLots * winBonus.LotMult, winBonus.LotMult);
            dash += "\n";
            dash += string.Format("  Max Lot:   {0}\n", tier.MaxLots);
            dash += string.Format("  Max Pos:   {0}", tier.MaxPositions);
            if (winBonus.ExtraPositions > 0)
                dash += string.Format(" → {0} (+{1})", effectiveMaxPositions, winBonus.ExtraPositions);
            dash += "\n";
            dash += "─────────────────────────\n";
            dash += string.Format("Cooldown:    {0}\n", cooldownStatus);
            dash += string.Format("Win Streak:  {0}\n", winStatus);
            dash += string.Format("Session:     {0}\n", session);
            dash += string.Format("Trend:       {0}\n", trend);
            dash += string.Format("Positions:   {0}/{1}\n", b, effectiveMaxPositions);
